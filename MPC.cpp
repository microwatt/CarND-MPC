  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // the Solver function below.
	  ...
	  for ( unsigned int t = 0; t < N; t++) {
		  fg[0] += CppAD::pow(vars[cte_start + t] - 0, 2);
		  fg[0] += CppAD::pow(vars[epsi_start + t] - 0, 2);
		  fg[0] += CppAD::pow(vars[v_start + t] - 100, 2);
	  }

	  //Mimimize use of actuators
	  ...
	  for (unsigned int t = 0; t < N-1; t++) {
		  ...
		  AD<double> v0 = vars[v_start + 1 - 1];
		  AD<double> delta0 = vars[delta_start + 1 - 1];
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //Setting limits of delta to [-25, 25] degress in radians
